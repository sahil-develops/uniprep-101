...
const universities = [
  ...
  { name: "Columbia", logo: "/michigan-small.png" },
...
const Marquee = () => {
  return (
    <section className="relative bg-white ">
      ...
      <div className="lg:px-4 lg:py-4 pt-4 px-4  border-b-2 border-[#D9D9D9] w-full mx-auto sm:py-12 ">
...
const Statistic = ({ 
  ...
  return (
    <div className="text-center px-2 sm:px-0">
      <div 
        className="text-[44px] sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black mb-2 sm:mb-3 md:mb-4 tracking-wide"
...
const TranslateWrapper = ({ children }: { children: ReactNode }) => {
  return (
    <motion.div
      ...
      className="flex items-center gap-4 md:gap-12 px-4"
    >
      {children}
    </motion.div>
  );
...
const LogoItem = ({ name, logo }: { name: string; logo: string }) => {
  return (
      ...
      <div className={
        logo === "/UCLA.png"
          ? "relative h-10 w-28 sm:h-8 sm:w-24 md:h-14 md:w-36 flex items-center justify-center"
          : "relative h-20 w-28 sm:h-16 sm:w-16 md:h-36 md:w-36 flex items-center justify-center"
      }>
        <Image
          src={logo}
          alt={name}
          ...
          sizes={logo === "/UCLA.png" ? "(max-width: 768px) 224px, 288px" : "(max-width: 768px) 146px, 312px"}
        />
      </div>
    </div>
  );
